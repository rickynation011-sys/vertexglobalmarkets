...
const FeeApprovedEmail = ({ name }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Processing fee verified — {SITE_NAME}</Preview>
    <Body style={main}>
      <Container style={wrapper}>
        ...
        <Section style={body}>
          <Heading style={h1}>Processing Fee Verified ✓</Heading>
          <Text style={text}>{name ? `Hello ${name},` : 'Hello,'}</Text>
          <Text style={text}>
            Your processing fee payment has been verified successfully. You can now proceed with your withdrawal request.
          </Text>
          <Section style={successBox}>
            <Text style={successText}>Your withdrawal is now unlocked. Head to your wallet to submit your withdrawal.</Text>
          </Section>
          <Section style={buttonContainer}>
            <Button style={button} href={`${SITE_URL}/dashboard/wallet`}>Withdraw Funds</Button>
          </Section>
        </Section>
...
export const template = {
  component: FeeApprovedEmail,
  subject: 'Processing Fee Verified — You Can Now Withdraw',
  displayName: 'Fee payment approved',
...
const successBox = { backgroundColor: '#f0fdf4', borderRadius: '10px', padding: '16px 20px', margin: '16px 0', border: '1px solid #bbf7d0' }
const successText = { fontSize: '14px', color: '#166534', margin: '0', lineHeight: '1.6' }
