...
interface AdminNewFeePaymentProps {
  userName?: string; userEmail?: string; totalProfit?: string; processingFee?: string; paymentMethod?: string; submittedAt?: string
}

const AdminNewFeePaymentEmail = ({ userName = 'A user', userEmail = '', totalProfit = '0.00', processingFee = '0.00', paymentMethod = 'Crypto', submittedAt = new Date().toISOString() }: AdminNewFeePaymentProps) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>New fee payment proof: ${processingFee} from {userName}</Preview>
    <Body style={main}>
...
export const template = {
  component: AdminNewFeePaymentEmail,
  subject: (data: Record<string, any>) => `New Fee Payment: $${data.processingFee || '0'} from ${data.userName || 'a user'}`,
  displayName: 'Admin: New Fee Payment Notification',
  previewData: { userName: 'John Doe', userEmail: 'john@example.com', totalProfit: '5,000.00', processingFee: '500.00', paymentMethod: 'BTC', submittedAt: new Date().toISOString() },
