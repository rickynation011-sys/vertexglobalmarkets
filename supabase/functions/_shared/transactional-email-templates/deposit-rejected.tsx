...
interface Props { name?: string; amount?: string; method?: string; reason?: string }

const DepositRejectedEmail = ({ name, amount, method, reason }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Your deposit has been declined — {SITE_NAME}</Preview>
    <Body style={main}>
        ...
        <Section style={body}>
          ...
          {reason && (
            <Section style={infoBox}>
              ...
              <Text style={infoText}>{reason}</Text>
            </Section>
          )}
          <Text style={text}>
            Please review the details and try submitting a new deposit, or contact our support team for assistance.
          </Text>
          <Section style={buttonContainer}>
            <Button style={button} href={`${SITE_URL}/dashboard/wallet`}>Submit New Deposit</Button>
          </Section>
        </Section>
...
export const template = {
  ...
  previewData: { name: 'John', amount: '500', method: 'USDT', reason: 'Invalid proof of payment provided.' },
...
const infoBox = { backgroundColor: '#fef2f2', borderRadius: '10px', padding: '16px 20px', margin: '16px 0', border: '1px solid #fecaca' }
const infoLabel = { fontSize: '13px', fontWeight: '600' as const, color: '#991b1b', margin: '0 0 4px', textTransform: 'uppercase' as const, letterSpacing: '0.5px' }
const infoText = { fontSize: '14px', color: '#991b1b', margin: '0', lineHeight: '1.6' }
