...
interface Props { userName?: string; userEmail?: string; subject?: string; category?: string; submittedAt?: string }

const AdminNewTicketEmail = ({ userName, userEmail, subject, category, submittedAt }: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>New support ticket — {SITE_NAME}</Preview>
    <Body style={main}>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => `🎫 New Support Ticket: ${data.subject || 'No subject'}`,
  displayName: 'Admin: new support ticket',
  previewData: { userName: 'Jane Doe', userEmail: 'jane@example.com', subject: 'Cannot withdraw funds', category: 'withdrawal', submittedAt: new Date().toISOString() },
...
const infoBox = { backgroundColor: '#faf5ff', borderRadius: '10px', padding: '16px 20px', margin: '16px 0', border: '1px solid #e9d5ff' }
const infoText = { fontSize: '14px', color: '#6b21a8', margin: '0 0 6px', lineHeight: '1.6' }
