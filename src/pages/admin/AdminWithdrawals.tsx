...
import { Search, CheckCircle, XCircle, ArrowUpRight, Loader2 } from "lucide-react";
...
const AdminWithdrawals = () => {
  ...
  const fetchWithdrawals = async () => {
    ...
    setWithdrawals(txData.map((t) => ({ ...t, profile: profileMap.get(t.user_id) ?? null })));
    setLoading(false);
  };

  useEffect(() => { fetchWithdrawals(); }, []);

  const handleApprove = async (tx: WithdrawalTx) => {
    ...
    try {
      ...
      const newBalance = Math.max(0, (tx.profile?.wallet_balance ?? 0) - tx.amount);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-foreground">Withdrawal Management</h1>
        <p className="text-muted-foreground text-sm">Process withdrawal requests — balances debit on approval</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card className="bg-card border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Pending</p><p className="text-2xl font-display font-bold text-warning">{withdrawals.filter(w => w.status === "pending").length}</p></CardContent></Card>
        <Card className="bg-card border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Approved</p><p className="text-2xl font-display font-bold text-success">{withdrawals.filter(w => w.status === "approved").length}</p></CardContent></Card>
        <Card className="bg-card border-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground">Total Paid</p><p className="text-2xl font-display font-bold text-foreground">${withdrawals.filter(w => w.status === "approved").reduce((s, w) => s + w.amount, 0).toLocaleString()}</p></CardContent></Card>
      </div>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={tx.id} className="border-b border-border/50 last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-4">
                      {tx.wallet_address ? (
                        <code className="text-[10px] font-mono text-muted-foreground break-all">{tx.wallet_address}</code>
